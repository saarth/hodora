...
/** Returns the Google OAuth consent URL for the caller to navigate the browser to (can't be `fetch`ed cross-origin). */
export const Route = createFileRoute("/api/cloud/google-drive/authorize")({
  server: {
    handlers: {
      POST: async () => {
        ...
        try {
          ...
          const { buildAuthUrl } = await import("@/lib/sync/google-drive.server");

          const redirectUri = `${new URL(request.url).origin}/api/cloud/google-drive/callback`;
